...
export function useSearch(initialText?: string) {
  ...
  useEffect(() => {
    ...
    setHistoryResults(
      history.filter((item) => item.query?.toLowerCase().includes(lowerSearchText))
    );
  }, [searchText, history]);
  ...
  async function addHistory(result: SearchResult) {
    ...
    if (newHistory.some((item) => item.query === result.query)) return;
    newHistory?.unshift({ ...result, isHistory: true });
    setHistory(newHistory);
